builder_choices_for_type <- function(type) {
  ...
  switch(type,
    matrix = c("Matrix" = "matrix", "Double matrix" = "double_matrix", "Multi matrix" = "multi_matrix"),
...
mod_graph_builder_ui <- function(id) {
  ns <- shiny::NS(id)
  shiny::div(
    class = "graph-builder-two-column",
    bslib::card(
      class = "graph-builder-card",
      bslib::card_header("Build Graph"),
      shiny::div(
        class = "graph-builder-params",
      ...
      shiny::div(
        class = "graph-builder-actions",
        shiny::textInput(ns("graph_name"), "Graph name", value = "network_graph"),
        shiny::div(
          class = "graph-builder-build-action",
          shiny::actionButton(ns("build"), "Build graph", class = "w-100")
        )
      )
    ),
    bslib::card(
