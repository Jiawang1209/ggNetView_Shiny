...
- The app opens on an Introduction tab that renders the project README directly.
- The app includes a Manual tab after Export that serves `package/ggNetView-manual/docs` through Shiny and embeds the real `manual/index.html` bookdown site.
...
- Data Hub lets users choose CSV/TSV/TXT files first, then explicitly click `Load data` before registering the uploaded table into the shared object registry. It can also load manual workflow examples for matrix, edge-table-with-module, adjacency, TOM-like, and starter graph workflows.
...
- `tests/run_shiny_upload_to_builder_smoke.R` guards the human upload path: uploaded files stay out of Graph Builder until `Load data` is clicked, then become selectable, build a graph, and show changed build parameters in the status output.
